taxdict = {
    "sales_tax": {
        "state": 0.0,
        "local": "Varies on certain items",
        "combined": "varies"
    },
    "property_tax": {
        "effective_rate": "0.0088 - 0.0107"
    },
    "federal_income_tax": {
        "brackets_2025": {
            "single": {
                "10%": {"min": 0, "max": 11925},
                "12%": {"min": 11926, "max": 48475},
                "22%": {"min": 48476, "max": 103350},
                "24%": {"min": 103351, "max": 197300},
                "32%": {"min": 197301, "max": 250525},
                "35%": {"min": 250526, "max": 626350},
                "37%": {"min": 626351, "max": "above"}
            },
            "married_filing_jointly": {
                "10%": {"min": 0, "max": 23850},
                "12%": {"min": 23851, "max": 96950},
                "22%": {"min": 96951, "max": 206700},
                "24%": {"min": 206701, "max": 394600},
                "32%": {"min": 394601, "max": 501050},
                "35%": {"min": 501051, "max": 751600},
                "37%": {"min": 751601, "max": "above"}
            },
            "head_of_household": {
                "10%": {"min": 0, "max": 17000},
                "12%": {"min": 17001, "max": 64850},
                "22%": {"min": 64851, "max": 103350},
                "24%": {"min": 103351, "max": 197300},
                "32%": {"min": 197301, "max": 250500},
                "35%": {"min": 250501, "max": 626350},
                "37%": {"min": 626351, "max": "above"}
            }
        }
    },
    "state_income_tax":{
        "brackets_2023": {
            "5%": {"min": 0, "max": 3750},
            "7%": {"min": 3751, "max": 9000},
            "9%": {"min": 9001, "max": 12000},
            "9.9%": {"min": 12001, "max": "above"}
        }
    },
    "other_taxes": {
        "medicare_tax": "1.45% (may be subject to additional tax if income exceeds certain thresholds)",
        "social_security_tax": "6.2% of earnings (capped at a certain income level)",
        "city_income_tax": "none",
        "value_added_tax": "not applicable"
    }
}